...
SD_Error SD_GetCardInfo(SD_CardInfo *cardinfo)
{
	...
	******************************************************/
	...
	else if(CardType == SDIO_HIGH_CAPACITY_SD_CARD)
	{
		...
		cardinfo->CardCapacity=(cardinfo->SD_csd.DeviceSize+1)/2;	//卡容量		单位MB
...
SD_Error SD_ReadMultiBlocks(uint8_t *readbuff, uint32_t ReadAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
	...
	******************************************************/
	...
	//等待DMA操作完成
	SDIO_ITConfig(SDIO_IT_RXOVERR|SDIO_IT_DTIMEOUT|SDIO_IT_DCRCFAIL|SDIO_IT_DATAEND|SDIO_IT_STBITERR, ENABLE);//配置中断
	SDIO_DMACmd(ENABLE);	//SDIO DMA使能 
	SD_DMA_RxConfig((uint32_t *)readbuff, (NumberOfBlocks * BlockSize));//配置DMA
...
static SD_Error CmdResp3Error(void)
{
	...
	while(!(status & (SDIO_FLAG_CCRCFAIL | SDIO_FLAG_CMDREND | SDIO_FLAG_CTIMEOUT)))//crc错误   响应超时   响应成功
		status = SDIO->STA;
