...
void Device_Usart6_ENABLE_Init(u32 bound)
{
    ...
    }
    {
        ...
        DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralToMemory;//数据传输方向  外设到储存器
        DMA_InitStructure.DMA_BufferSize = BSP_USART6_DMA_RX_BUF_LEN;//数据接收缓冲
        DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;//
        DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;//设置DMA内存递增
        DMA_InitStructure.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Byte;//设置DMA数据宽度 一个字节
        DMA_InitStructure.DMA_MemoryDataSize = DMA_MemoryDataSize_Byte;//
        DMA_InitStructure.DMA_Mode = DMA_Mode_Normal;//
        DMA_InitStructure.DMA_Priority = DMA_Priority_Medium;
        DMA_InitStructure.DMA_FIFOMode = DMA_FIFOMode_Disable;//禁止两个memory相互访问
