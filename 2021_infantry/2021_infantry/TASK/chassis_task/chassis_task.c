...
static void mecanum_calc(float vx, float vy, float vz, float speed[], chassis_move_t *power_ctrl); //全向算法
...
static void mecanum_calc(float vx, float vy, float vz, float speed[], chassis_move_t *power_ctrl)
{
    ...
    rotate_ratio_fr 	= ((WHEELBASE + WHEELTRACK) / 2.0f - chassis_move.rotate_x_offset + chassis_move.rotate_y_offset) / RADIAN_COEF; //6.63
    rotate_ratio_fl 	= ((WHEELBASE + WHEELTRACK) / 2.0f - chassis_move.rotate_x_offset - chassis_move.rotate_y_offset) / RADIAN_COEF;
    rotate_ratio_bl = ((WHEELBASE + WHEELTRACK) / 2.0f + chassis_move.rotate_x_offset - chassis_move.rotate_y_offset) / RADIAN_COEF;
    rotate_ratio_br = ((WHEELBASE + WHEELTRACK) / 2.0f + chassis_move.rotate_x_offset + chassis_move.rotate_y_offset) / RADIAN_COEF;
    ...
    else if((Super_power.volt < 17000 || Shift_Flag == 2) && Ctrl_Flag == 0)
    {
